---
title: 高并发限流
url: /docs/notes/d-205f3859abe469be/d-e303a4ad6f96797b/n-976be63085041031/
draft: false
showDate: false
...
showEdit: false
obsidianSource: 面试/场景题/高并发限流.md
---

# 高并发限流解决方案 {#h-07aa1300a9c68e60}
...
# 2. 为什么要限流 {#h-353bdff4af0c279d}

限流主要是保证在高并发场景下，可以拒绝掉一部分请求，避免因过载导致系统崩溃或性能下降，从而保证服务能够健康稳定的运行。具体来看，主要有以下一些原因：

## 2.1 防止系统过载 {#h-d4e4afb92506afbf}


## 2.2 提升系统稳定性 {#h-0226c18c253646bc}


## 2.3 应对突发流量 {#h-f2ca4302c05d68e0}

- 瞬时高峰处理：在特定时间段（如促销活动、突发事件等），请求量可能突然增加。限流可以控制请求速率，平稳地处理瞬时高峰。

- 恶意请求防护：限流可以防止恶意用户或攻击者通过大量请求来消耗系统资源，提高系统的安全性。

# 3. 限流基本算法 {#h-c71d251de34a4a6e}

## 3.1 固定窗口限流 {#h-5b5fc259303f8951}

### 3.1.1 算法原理 {#h-248b86dc66546c9b}
...
5. 当时间窗口结束时，重置计数器
![image.png](/obsidian/cada4fc83a9c96ac/image.png)

如上图，假设时间窗口长度为1s，限流阈值为3，每秒内超过3个以上的请求都会被拒绝

### 3.1.3 优点 {#h-214f3fd68ac211ae}

算法实现非常简单，易于实现和理解

### 3.1.4 缺点 {#h-6af2d3a51885f695}
...
![Pasted image 20260907170521.png](/obsidian/bbb78fc6400efae8/Pasted%20image%2020260907170521.png)
### 3.1.5 适用场景 {#h-bda6a14faf85fdbe}

固定窗口算法适合在请求速率有明确要求且流量相对稳定的场景中使用。然而，对于应对突发流量和请求分布不均匀的情况，该算法可能表现不足，此时需要考虑使用其他更灵活的限流算法。

## 3.2 滑动窗口限流 {#h-80010d460f04b44d}

### 3.2.1 算法原理 {#h-e0f0eaab5af6abd6}
...
![image (1).png](/obsidian/e5ebc5be3cce7d10/image%20%281%29.png)

这样的话，在固定窗口中会出现的临界问题，比如0.5s和1.5s之假设来了10个请求，由于滑动窗口会统计到这个时间段，所以多余请求就会被拒绝
### 3.2.3 优点 {#h-3e6ebb9a694d323f}
...
### 3.2.5 适用场景 {#h-1d25e7492b640b7d}

滑动窗口同样也是适合流量相对稳定的场景

## 3.3 漏斗限流 {#h-875f09a6d92adc14}

### 3.3.1 算法原理 {#h-0a91c3e7fcb9e9c2}
...
## 4.1 基于中心化的限流方案 {#h-36295a7539c37886}

### 4.1.1 实现原理 {#h-ae40539c313fb404}
...
### 4.1.3 存在的问题 {#h-9c6c902838d32bf4}
...
## 4.2 基于负载均衡的分布式限流方案 {#h-c9afb0d2e263b128}
...
### 4.2.1 存在的问题 {#h-fcab025d1f0f13ed}
...
# 6. Java版本demo代码 {#h-7b698fd1b7bd3293}
