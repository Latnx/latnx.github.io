---
title: Go
url: /docs/notes/d-d69eb5fbdf57b275/n-37e395229cd6ac88/
draft: false
showDate: false
...
showEdit: false
obsidianSource: 面试题/Go.md
---

## 基础 {#h-3a13bd2148070e2c}
### 1. 相比较于其他语言, Go 有什么优势或者特点？ {#h-ba19a39319f49624}
...
### 2. go是面向对象的吗 {#h-05daa8dab05fd9b7}

- 封装：Go语言里面字段首字母大小写来决定字段是否可以被外包访问
- 继承：Go语言里面用组合结构体的方式 或 接口继承来实现
- 多态：Go语言中通过接口来实现多态，不同的类型实现对应接口，然后调用接口变量的方法，结果取决于接口存储的对应类型的方法

### 3. golang 中 make 和 new 的区别？（基本必问） {#h-ecf7a5a7ad13f85b}

- make 只能用来分配及初始化类型为 slice、map、chan 的数据。new 可以分配任意类型的数据；
- new 分配返回的是指针，即类型 \*Type。make 返回数据类型本身，即 Type；
- new 分配的空间被清零。make 分配空间后，会进行初始化；

### 4. 数组和切片的区别 （基本必问），切片怎么扩容 {#h-d689d2d2262a8fdb}

数组是值类型，长度固定​
切片是引用类型，长度不固定，可以动态扩容
newcap = oldcap+(oldcap+3\*256)/4
![Pasted image 20260527215720.png](/obsidian/dff875075936f621/Pasted%20image%2020260527215720.png)

### 5. for i, v := range 切片，v地址会变化吗 {#h-05da61234633c8cb}

地址不会发生变化，但是该地址的值是变化的，每遍历到一个元素，就把该元素值就会写到该地址​
PS：在最新版本Go 1.22中，v的地址会变化的，也就是不再共享变量了，知道这点的话，在面试一定要提出来，展示自己的技术面广

### 6. go defer，多个 defer 的顺序，defer 在什么时机会修改返回值？（defer 和 return） {#h-20759daab735943b}

#### 5.1 defer 的执行顺序 {#h-da9cbb9ea3612deb}

defer的执行顺序类似于栈，是LIFO，先调用的defer语句后执行。

#### 5.2 defer在什么时机会修改返回值 {#h-832d5cbeee83d8a2}

只有“命名返回值”会被 defer 修改。
因为 defer 操作的是同一个返回变量。

### 7. uint 类型溢出 {#h-939d2cbc03978441}
...
### 8. 介绍 rune 类型 {#h-35fc1cde5ce246d7}
...
```
### 9. Go中两个Nil可能不相等吗？ {#h-e94b648eb85955aa}
...
### 10. golang 中解析 tag 是怎么实现的？反射原理是什么？ {#h-ff99ba10dda28355}
...
### 13. 空 struct{} 占用空间么？ {#h-9cede62bff03975b}

空结构体 struct{} 实例不占据任何的内存空间。

### 14. go语言中空 struct{} 的用途？ {#h-189a5c36f0e94d0a}
...
### 15. go中"\_"的作用 {#h-39a9596fb41ff979}

1. import中的下滑线用于执行导入包下的所有init函数​
2. 代码体中的下划线用于忽略返回值

### 16. Go 闭包 {#h-4ebde56adc309556}

闭包实际上就是匿名函数 + 引用环境(捕获的变量)​
Go 中闭包**总是以引用方式捕获变量**

### 17. Go 多返回值怎么实现的？ {#h-8d6a55019140b3c6}

函数调用时，调用者会在自己的栈帧中预先分配返回值的存储空间，然后将地址传递给被调用函数。被调用函数执行完后，直接将返回值写入这些位置。

### 18. Go 语言中不能比较的类型如何比较是否相等？ {#h-8d8e65a8714a1a07}
...
### 2、context 使用场景和用途？（基本必问） {#h-c470960007ade587}
...
## Chennel {#h-c99e2661d411f38f}
### 1. channel 是否线程安全？锁用在什么地方？ {#h-9a41fc0915c32c5e}

一般来说，我们对channel就只有读，写，关闭三种操作，这三种操作，channel底层数据结构都用同一把runtime.Mutex来进行保护
### 2. channel 的底层实现原理 （数据结构） {#h-4a29eda31e485555}
...
### 3. nil、关闭的 channel、有数据的 channel，再进行读、写、关闭会怎么样？（各类变种题型） {#h-f4936dbda54710d2}
...
### 4. 对channel 进行读写数据的流程是怎样的 {#h-6b0fb49ecc829d74}
...
### 5. select的底层原理 {#h-8286e7a2dffec422}
...
## Map {#h-3d097c3d80191408}

### 1. map 使用注意的点，是否是并发安全的？ {#h-b7321daeec5effc8}
...
### 2. map 循环是有序的还是无序的？ {#h-a8959afa97ac4da2}
...
### 3. map 如何顺序读取？ {#h-cec0b13801001227}

如果想顺序遍历map,先把key放到切片排序,再按照key的顺序遍历map

### 4. map 中删除一个 key，它的内存会释放么？ {#h-c49d6e4fadd6f42f}

不会释放，删除一个key，可以认为是标记删除，只是修改key对应内存位置的值为空，并不会释放内存，只有在置空这个map的时候，整个map的空间才会被垃圾回后释放

### 5. 怎么处理对 map 进行并发访问？有没有其他方案？ 区别是什么？ {#h-65b830131ab82128}
...
### 6. nil map 和空 map 有何不同？ {#h-3fe676fc503251e7}
...
### 7. map 的数据结构是什么？是怎么实现扩容 {#h-bd11b7c78f4be2be}


<img src="/obsidian/657e3a8a4f5a224d/Pasted%20image%2020260529144657.png" alt="Pasted image 20260529144657.png" width="479" loading="lazy" style="max-width:100%;height:auto">
...
### 8. map 的 key 为什么得是可比较类型的？ {#h-54d351bb5ef0dc3a}
...
## sync.Map {#h-914e8d29d0641dfc}

### 1. sync.Map的底层原理 {#h-aa5e5c8b5ea3361c}
...
### 2. read map和dirty map之间的关联？ {#h-b6a3af23b4e1c146}
...
### 3. 为什么要设计nil和expunged状态？ {#h-95bbff5bbdd699ae}
...
### 4. sync.Map 适用的场景？ {#h-38fa0f7ebce94bc9}

sync.Map 是适用于读多、更新多、删多、写少的场景

### 5. 你认为sync.Map有啥不足吗？ {#h-376d594d94ef2c07}
...
### 6. 补充知识——分段锁map是什么 {#h-73008049bb22aabc}

## GMP {#h-a3bb07ecce667a4a}
![Pasted image 20260529223114.png](/obsidian/e404e6903ca43a0b/Pasted%20image%2020260529223114.png)
### 1. 什么是 GMP？（必问）调度过程是什么样的？（对流程熟悉，要求更高，问的较少） {#h-e10e9d7c21464e3e}
...
### 2. GMP能不能去掉P层？会怎么样？ {#h-79397d7aa8a018b4}

- 每个 **P 有自己的本地队列**，大幅度的减轻了对全局队列的直接依赖，所带来的效果就是锁竞争的减少。而 GM 模型的性能开销大头就是锁竞争。​

- 每个 P 相对的平衡上，**在 GMP 模型中也实现了 Work Stealing 算法**，如果 P 的本地队列为空，则会从全局队列或其他 P 的本地队列中窃取可运行的 G 来运行，减少空转，提高了资源利用率。

### 3. M 和 P 的数量问题？ {#h-0acde9fef4079d91}
...
### 4. 进程、线程、协程有什么区别？ {#h-f6dfdacc32d8fd33}
...
### 5. 抢占式调度是如何抢占的？ {#h-49d7a0c4be9e476a}
...
atomic -> 原子操作（更底层）
```
### 1. 除了 mutex 以外还有那些方式安全读写共享变量？ {#h-a512e2de751e50bb}
...
### 2. Go 如何实现原子操作？ {#h-73bad45331d2589c}
...
```
### 3. 原子操作和锁的区别 {#h-ef61e3c3021d3ed7}
...
### 4. Mutex 是悲观锁还是乐观锁？悲观锁、乐观锁是什么？ {#h-cc507a0963f937be}
...
### 5. 互斥锁mutex底层是怎么实现的？ {#h-d60cfc5c19cc0fc1}

mutex底层是通过原子操作加信号量来实现的，
- 通过atomic 包中的一些原子操作来实现锁的锁定
- 通过信号量来实现协程的阻塞与唤醒

### 6. Mutex 有几种模式？ {#h-29c9c0b890f915e4}
...
### 7. 在Mutex上自旋的goroutine 会占用太多资源吗 {#h-4d133da914c3b354}
...
### 8. 读写锁底层是怎么实现的 {#h-5198dc87762c6d42}
...
### 9. Mutex 已经被一个 Goroutine 获取了, 其它等待中的 Goroutine 们只能一直等待。那么等这个锁释放后，等待中的 Goroutine 中哪一个会优先获取 Mutex 呢? {#h-0e00af0ef8f6c4fc}
...
### 10. waitgroup 是怎样实现协程等待？ {#h-53c3be97e59e5e50}
...
### 11. sync.Once 的原理，是怎样保证代码段只执行 {#h-e1c45cb7d5f9e4c3}

内部维护了一个标识位，当它 == 0 时表示还没执行过函数，此时会加锁修改标识位，然后执行对应函数。后续再执行时发现标识位 != 0，则不会再执行后续动作了

## 并发 {#h-b3d9e81aefa8cc4d}

### 1. 怎么控制并发数？ {#h-c598a6095c9e4109}
...
```
### 2. 多个 goroutine 对同一个 map 写会 panic，异常是否可以用 defer 捕获？ {#h-6acabe536556269f}
Go 语言的错误分三种 error, panic 和 fatal error

map会检测是否存在并发写，如果检测到并发写会触发Fatal error ，Fatal error是属于系统出发的严重错误，无法被recover()，程序会直接退出

### 3. 如何优雅的实现一个 goroutine 池（百度、手写代码） {#h-f497a2b57ece5ec6}


### 4. select 可以用于什么？ {#h-fddd5f68c6c5160c}
...
### 5. 主协程如何等其余协程完再操作？ {#h-9c6528ce211673f9}

可以用sync.WaitGroup或channel来实现协程等待


## GC {#h-ac203598a47e2348}
### 1. go gc 是怎么实现的？（必问） {#h-cc77a222a53ca3c0}

go语言的gc策略是采用三色标记法。
但是单纯的三色标记会带来STW，到执行率不高，所以在1.8版本之后，采用了三色标记法配合**混合写屏障**技术来实现gc。

#### **Go gc经历了那几个版本？** {#h-5738f63f7b04d4b1}

1.3版本前：普通标记清除法，整个gc过程需要启动 STW，效率极低 ​
![Pasted image 20260527230019.png](/obsidian/02d75940fd16095d/Pasted%20image%2020260527230019.png)
...
#### **三色标记法过程是怎样的？有什么问题？** {#h-5992a3adab7acd54}
...
## 内存相关： {#h-1c4b5b719af3b390}
### 1. 谈谈内存泄露，什么情况下内存会泄露？怎么定位排查内存泄漏问题？ {#h-3cf4f0efa8d3086b}
...
### 2. 知道 golang 的内存逃逸吗？什么情况下会发生内存逃逸？ {#h-265f13a475651749}
...
3. goroutine 使用局部变量
大量的内存逃逸会给gc带来压力
### 3. 请简述 Go 是如何分配内存的？ {#h-e601b6084b2ccdd6}
...
### 4. Channel 分配在栈上还是堆上？哪些对象分配在堆上，哪些对象分配在栈上？ {#h-2e439360832cb500}
...
### 5. 介绍一下大对象小对象，什么情况下会导致GC压力大？ {#h-3972e4cee3a6f91f}

- go语言中小于等于 32k 的对象就是小对象，其中小于16B 的是微小对象，其它都是大对象。​

- 当有大量小对象逃逸到堆上，或者有巨大的元素类型为指针的map和slice的情况下，GC压力会较大

## Go代码性能优化：​ {#h-6d8298a6e1af1847}
### 1. 你知道Go的哪些性能优化手段 {#h-cb2bb14f9e60dfab}
...
## gin框架相关 {#h-80a8f00beb493d31}
### 1. Gin框架的路由实现原理？ {#h-fb18222624aa6046}

在 gin 框架中，使用的正是压缩前缀树的数据结构.

- gin 的每种方法 (POST, GET ...) 都有自己的一颗 路由树。​
- 当 gin 收到客户端的请求时, 会去 路由树 里根据 URL 找到相关的 处理函数（handler）
### 2. gin框架的路由数据结构为什么使用压缩前缀树，而不用hashmap {#h-095e27a6780380b6}
